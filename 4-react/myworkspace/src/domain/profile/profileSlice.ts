...
// redux store(리덕스 저장소)에 하나의 state 관리하고 처리할 수 있는 모듈
...
export const profileSlice = createSlice({
  ...
  // state변경함수 목록
  reducers: {
    // action type : profile/saveProfile 에 해당하는 reducer함수
    // slice명/reducer함수명
    // 함수명: (기존state변수명, action변수명) => {state변경처리}
...
// action creator는 action객체를 생성하는 함수
